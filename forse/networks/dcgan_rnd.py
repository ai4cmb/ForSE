...
class DCGAN:
     ...
     def train(self, epochs, patches_file, batch_size=32, save_interval=100, swap=None, seed=4324):
         self.build_gan()
         X_train, X_test, Y_train, Y_test = load_training_set(patches_file, seed=seed, ratio=False)
         ...
         for epoch in range(epochs):
             ...
             if swap:
                 ...
                 for i in range(swap):
                     ...
                     if swap_fake[i] < half_batch:
                         ...
                         #target_fake[swap_fake[i]] = np.random.uniform(0.7, 1.0)
             #tot_img = np.concatenate((imgs, gen_imgs), axis=0)
             #tot_target = np.concatenate((target_real, target_fake), axis=0)
             #shuffle = np.arange(len(tot_img))
             #np.random.shuffle(shuffle)
             #tot_img = tot_img[shuffle]
             #tot_target = tot_target[shuffle]
             ...
             #d_loss = self.discriminator.train_on_batch(tot_img, tot_target)
             ...
             if epoch % (save_interval) == 0:
                 print(epoch)
                 #d_loss = self.discriminator.train_on_batch(tot_imgs, tot_target)
                 #d_loss_real = self.discriminator.train_on_batch(imgs, target_real)
                 #d_loss_fake = self.discriminator.train_on_batch(gen_imgs, target_fake)
